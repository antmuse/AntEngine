...
u32 Http1Parser::GMAX_HEAD_SIZE = HTTP_MAX_HEADER_SIZE;

Http1Parser::Http1Parser(HttpLayer* conn, EHttpParserType tp) :
    mConner(conn), mPType(tp), mMsg(nullptr), mHttpError(HPE_OK) {
    clear();
}
...
#ifdef DDEBUG
s32 TestHttpReceive(Http1Parser& mMsg) {
    usz tlen;
    usz used;
...
const s8* Http1Parser::parseValue(const s8* curr, const s8* end, StringView* out, s32& omax, s32& vflag) {
    vflag = 0;
    s32 used = 0;
    StringView kname;
    StringView kval;
...
void Http1Parser::addUrlQuery(StringView& key, StringView& val) {
    DASSERT(mMsg);
...
usz Http1Parser::parseBuf(const s8* data, usz len) {
...
const s8* Http1Parser::parseBoundBody(const s8* curr, const s8* end, StringView& tbody) {
    u32 cmpbyte = 0;
...
bool Http1Parser::needEOF() const {
...
bool Http1Parser::shouldKeepAlive() const {
...
void Http1Parser::pauseParse(bool paused) {
...
bool Http1Parser::isBodyFinal() const {
    return mState == PS_MSG_DONE;
}

const s8* Http1Parser::getErrStr(EHttpError it) {
...
void Http1Parser::msgBegin() {
    if (mConner->getWebsite()) {
        if (mMsg) {
            mHttpError = HPE_CB_MsgBegin;
            DLOG(ELL_ERROR, "Http1Parser::msgBegin>> fail pre_msg check, %s", getErrStr());
            mConner->postClose();
            return; // error
        }
        mMsg = new HttpMsg(mConner);
    }
}

void Http1Parser::msgEnd() {
    DASSERT(mMsg);
    if (mMsg) {
        if (mMsg->getEvent()) {
...
s32 Http1Parser::headDone() {
    DASSERT(mMsg);
    if (mMsg) {
        mMsg->mFlags = mFlags;
        if (!mMsg->getEvent()) {
            mConner->getWebsite()->createMsgEvent(mMsg);
        }
        if (EE_OK != mMsg->getEvent()->onReqHeadDone(mMsg)) {
            DLOG(ELL_ERROR, "Http1Parser::headDone>> fail onReqHeadDone");
            mConner->postClose();
        }
    }
    return 0;
...
void Http1Parser::chunkHeadDone() {
    DASSERT(mMsg);
    if (mMsg) {
        mMsg->mFlags = mFlags;
...
void Http1Parser::msgBody() {
    if (mMsg) {
        mMsg->getEvent()->onReqBody(mMsg);
    }
}

void Http1Parser::msgError() {
    if (mMsg) {
        if (mMsg->getEvent()) {
...
void Http1Parser::chunkMsg() {
    if (mMsg) {
        mMsg->getEvent()->onReqBody(mMsg);
    }
}

void Http1Parser::chunkDone() {
    if (mMsg) {
...
void Http1Parser::msgClose() {
    if (mMsg) {
        ...
        s32 cnt = mMsg->drop();
        mMsg = nullptr;
        DLOG(ELL_INFO, "onClose>> resp_msg_grab= %d", cnt);
    }
}
