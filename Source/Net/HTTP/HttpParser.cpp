...
u32 HttpParser::GMAX_HEAD_SIZE = HTTP_MAX_HEADER_SIZE;
...
do {                                                                \
  ...
  if (LIKELY(mCall##FOR)) {                                         \
    mState = p_state;                                               \
    if (UNLIKELY(0 != mCall##FOR(*this))) {                         \
      mReadSize = nread; mHttpError = (HPE_CB_##FOR);               \
    }                                                               \
...
const s8* HttpParser::parseValue(const s8* curr, const s8* end, StringView* out, s32& omax, s32& vflag) {
    vflag = 0;
    s32 used = 0;
    StringView kname;
    StringView kval;
...
usz HttpParser::parseBuf(const s8* data, usz len) {
    ...
    if (len == 0) {
        ...
        case s_body_identity_eof:
            if (mCallMsgComplete) {
                if (0 != mCallMsgComplete(*this)) {
                    mHttpError = HPE_CB_MsgComplete;
                }
            }
            return 0;
    ...
    for (p = data; p != end; ++p) {
        ...
        case s_res_status:
            if (ch == CR || ch == LF) {
                ...
                DASSERT(mHttpError == HPE_OK);
                if (tstat.mData) {
                    if (LIKELY(mCallStatus)) {
                        tstat.mLen = p - tstat.mData;
                        mState = p_state;
                        if (UNLIKELY(0 != mCallStatus(*this, tstat.mData, tstat.mLen))) {
                            mReadSize = nread;
                            mHttpError = (HPE_CB_Status);
                        }
                        if (UNLIKELY(mHttpError != HPE_OK)) {
                            return (p - data + 1);
                        }
...
#undef XX
        ...
        case s_req_fragment:
        {
            ...
            if (1 == turl.mLen && turl.mData) {
                ...
                if (LIKELY(mCallURL)) {
                    ...
                    mState = p_state;
                    if (UNLIKELY(0 != mCallURL(*this, turl.mData, turl.mLen))) {
                        mReadSize = nread;
            ...
            for (; p < end; p++) {
                ...
                if (ch == CR) {
                    ...
                    if (headkey.mData && headval.mData) {
                        ...
                        if (LIKELY(mCallHeader)) {
                            mState = p_state;
                            if (UNLIKELY(0 != mCallHeader(*this, headkey, headval))) {
                                mReadSize = nread;
                                mHttpError = HPE_CB_HeaderValue;
                            }
                            p_state = (EPareState)(mState);
                ...
                if (ch == LF) {
                    ...
                    if (headkey.mData && headval.mData) {
                        ...
                        if (LIKELY(mCallHeader)) {
                            ...
                            /* We either errored above or got paused; get out */
                            if (UNLIKELY(mHttpError != HPE_OK)) {
                                return (p - data); //without current byte
                            }
                        }
                        headkey.set(nullptr, 0);
                        headval.set(nullptr, 0);
        ...
        case s_header_value_discard_lws:
        {
            if (ch == ' ' || ch == '\t') {
                ...
                p_state = (s_header_field_start);
                //CALLBACK_DATA_NOADVANCE(HeaderValue);
                DASSERT(mHttpError == HPE_OK);
                if (headkey.mData && headval.mData) {
                    headval.mLen = p - headval.mData;
                    if (LIKELY(mCallHeader)) {
        ...
        case s_headers_almost_done:
        {
            ...
            if (mFlags & F_HEAD_DONE) {
                ...
                if (mFlags & (F_CHUNKED | F_TAILING)) {
                    ...
                    CALLBACK_NOTIFY_NOADVANCE(ChunkComplete);
                    goto GT_REPARSE;
                }
            }
            ...
            /* Set this here so that mCallHeadComplete() callbacks can see it */
            if ((mFlags & F_UPGRADE) &&
                (mFlags & F_CONNECTION_UPGRADE)) {
             ...
             */
            if (mCallHeadComplete) {
                switch (mCallHeadComplete(*this)) {
                case 0:
                    break;
            ...
            if (mContentLen == 0) {
                ...
                if (tbody.mData) {
                    if (LIKELY(mCallBody)) {
                        ...
                        mState = p_state;
                        if (UNLIKELY(0 != mCallBody(*this, tbody.mData, tbody.mLen))) {
                            mReadSize = nread;
...
const s8* HttpParser::parseBoundBody(const s8* curr, const s8* end, StringView& tbody) {
    u32 cmpbyte = 0;
...
const s8* HttpParser::getMethodStr(EHttpMethod m) {
    return m < DSIZEOF(GHTTP_METHOD_STRS) ? GHTTP_METHOD_STRS[m] : "<unknown>";
}

const s8* HttpParser::getStatusStr(EHttpStatus s) {
    switch (s) {
...
void HttpParser::init(EHttpParserType tp, void* user) {
    memset(this, 0, DOFFSET(HttpParser, mUserData));
    mUserData = user;
    mType = tp;
    mState = (tp == EHTTP_REQUEST ? s_start_req : (tp == EHTTP_RESPONSE ? s_start_resp : s_start_resp_or_req));
    mHttpError = HPE_OK;
}

const s8* HttpParser::getErrorStr(EHttpError err) {
    DASSERT(((usz)err) < DSIZEOF(HttpStrErrorTab));
    return HttpStrErrorTab[err].name;
}

const s8* HttpParser::getErrorBrief(EHttpError err) {
    DASSERT(((usz)err) < DSIZEOF(HttpStrErrorTab));
    return HttpStrErrorTab[err].description;
}

void HttpParser::pauseParse(bool paused) {
...
bool HttpParser::isBodyFinal() const {
...
s32 HttpParserURL::parseHost(const s8* buf, s32 found_at) {
...
s32 HttpParserURL::parseURL(const s8* buf, usz buflen, s32 is_connect) {
    if (buflen == 0) {
        return 1;
    }
