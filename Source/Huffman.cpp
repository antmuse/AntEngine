...
static HuffEncodeNode HuffEncodeTable[256] = {
...
static HuffEncodeNode  HuffEncodeTableLow[256] = {
    {0x00001ff8, 13}, {0x007fffd8, 23}, {0x0fffffe2, 28}, {0x0fffffe3, 28},
    {0x0fffffe4, 28}, {0x0fffffe5, 28}, {0x0fffffe6, 28}, {0x0fffffe7, 28},
    {0x0fffffe8, 28}, {0x00ffffea, 24}, {0x3ffffffc, 30}, {0x0fffffe9, 28},
    {0x0fffffea, 28}, {0x3ffffffd, 30}, {0x0fffffeb, 28}, {0x0fffffec, 28},
...
#ifdef DOS_64BIT
#ifdef DENDIAN_BIG
#define AppEncodeBuffer(dst, buf)  (*(u64*)(dst) = (buf))
#else
#define AppEncodeBuffer(dst, buf) (*(u64*)(dst) = AppSwap64(buf))
#endif
#else
#ifdef DENDIAN_BIG
#define AppEncodeBuffer(dst, buf) (*(u32*)(dst) = (buf))
#else
#define AppEncodeBuffer(dst, buf) (*(u32*)(dst) = AppSwap32(buf))
#endif
#endif


usz AppHuffEncode(const u8* src, usz len, u8* dst, bool lower) {
    HuffEncodeNode* next;
    HuffEncodeNode* table = lower ? HuffEncodeTableLow : HuffEncodeTable;
    usz hlen = 0;
    usz buf = 0;
    usz pending = 0;
    usz code;
    ...
    buf |= ((usz)-1) >> pending;
    pending = AppAlignSize(pending, 8);
...
static HuffDecodeNode HuffDecodeTable[256][16] = {
    /* 0 */
    {
...
static inline bool AppHuffDecodeBits(u8* state, u8* ending, u32 bits, u8** dst) {
    HuffDecodeNode code = HuffDecodeTable[*state][bits];
    if (code.next == *state) {
        return false; // fail
    }
    if (code.emit) {
...
bool AppHuffDecode(const u8* src, usz len, u8** dst, u8* state, bool last) {
    u8 ch = 0;
    u8 ending = 1;
    const u8* end = src + len;
    while (src != end) {
        ch = *src++;
        if (!AppHuffDecodeBits(state, &ending, ch >> 4, dst)) {
            DLOG(ELL_ERROR, "http huffman decoding error at state %d: bad code 0x%X", *state, ch >> 4);
            return false; // fail
        }
        if (!AppHuffDecodeBits(state, &ending, ch & 0xf, dst)) {
            DLOG(ELL_ERROR, "http huffman decoding error at state %d: bad code 0x%X", *state, ch & 0xf);
            return false; // fail
        }
    }
    if (last) {
        if (!ending) {
            DLOG(ELL_ERROR, "http huffman decoding error: incomplete code 0x%X", ch);
            return false;
        }
        *state = 0; // finished
    }
