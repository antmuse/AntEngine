...
*/
static const u16 G_CRC16TABLE[256] = {
...
static const u64 G_CRC64TABLE[256] = {
...
u16 CheckCRC16::add(const void* buffer, u32 len) {
    if (buffer) {
        u16 ret = mResult;
        const u8* buf = (const u8*)buffer;
        while (len--) {
            ret = (ret << 8) ^ G_CRC16TABLE[(ret >> 8) ^ *buf++];
        }
        mResult = ret;
    }
    return mResult;
...
u32 CheckCRC32::add(const void* buffer, u64 len) {
    u32 ret = mResult;
    if (buffer) {
        const u8* buf = (const u8*)buffer;
        while (len--) {
            ret = G_CRC32TABLE[(ret ^ *buf++) & 0xFF] ^ (ret >> 8);
        }
        mResult = ret;
    }
...
u64 CheckCRC64::add(const void* buffer, u64 len) {
    u64 ret = mResult;
    if (buffer) {
        const u8* buf = (const u8*)buffer;
        while (len--) {
            ret = G_CRC64TABLE[(u8)ret ^ *buf++] ^ (ret >> 8);
        }
        mResult = ret;
    }
    return mResult;
