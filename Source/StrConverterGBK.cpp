//details Check GBK character you could do code >= 0x8000 && GTB_GBK_UTF16[code - 0x8000] != 0
...
//#define DCOMP_BYTE(x, y) ((u8)(x) << 8 | (u8)(y))
DFINLINE u16 AppCompU16(u16 x, u16 y) {
    return ((u8)(x) << 8 | (u8)(y));
}



static const u16* _pTrad2Simp_gbk;
static const u16* _pTrad2Simp_utf16;
static const u16* GTB_PLAIN_GBK;
static const u16* GTB_PLAIN_UTF16;
static const s8* _pUpper2Lower;
static const s8* _pLower2Upper;
static const s8* GTB_PLAIN;
static const u16* GTB_GBK_UTF16;
static const u16* GTB_UTF16_GBK;

static const u16* _pTns;
static u16 _tns_size;
static const u16* _pGbk2utf16_2;
static u16 _gbk2utf16_2_size;
static const u16* _pGbk2utf16_3;
static u16 _gbk2utf16_3_size;

const u16* _initTns();
const u16* AppInitGbk2UTF16_2();
const u16* AppInitGbk2UTF16_3();
const u16* _initTrad2Simp_utf16();
const u16* _initTrad2Simp_gbk();
const u16* AppInitPlainUTF16();
const u16* AppInitPlainGBK();
const s8* AppInitUpper2Lower();
const s8* AppInitLower2Upper();
const s8* AppInitPlain();
const u16* AppInitGBKUTF16();
const u16* AppInitUTF16GBK();

void AppStrConverterInit() {
    _pTns = _initTns();
    _pGbk2utf16_2 = AppInitGbk2UTF16_2();
    _pGbk2utf16_3 = AppInitGbk2UTF16_3();
    _pTrad2Simp_gbk = _initTrad2Simp_gbk();
    _pTrad2Simp_utf16 = _initTrad2Simp_utf16();
    GTB_PLAIN_GBK = AppInitPlainGBK();
    GTB_PLAIN_UTF16 = AppInitPlainUTF16();
    _pUpper2Lower = AppInitUpper2Lower();
    _pLower2Upper = AppInitLower2Upper();
    GTB_PLAIN = AppInitPlain();
    GTB_GBK_UTF16 = AppInitGBKUTF16();
    GTB_UTF16_GBK = AppInitUTF16GBK();
}

const u16* _initTns() {
    static const u16 _tns[] = {
...
const u16* _initTrad2Simp_utf16() {
    u16* gbk2utf16 = (u16*)malloc(0x10000);
    u16 n;
    u16 d;
    u16 c = 0;
    n = _gbk2utf16_2_size;
...
const u16* _initTrad2Simp_gbk() {
    static u16 t2s[0x8000];
    u16 n;
    u16 c = 0;

    for (c = 0; c < 0x8000; c++) {
        t2s[c] = AppSwap16(c | 0x8000);
    }
    n = _tns_size;
    ...
    for (c = 0; c < n; c += 2) {
        ...
        if (!((_pTns[c] >> 8) >= 0xA1 && (_pTns[c] >> 8) < 0xF8
            && (_pTns[c] & 0xFF) >= 0xA1)) {
            t2s[_pTns[c] - 0x8000] = AppSwap16(_pTns[c + 1]);
        }
    }
...
const u16* AppInitPlainGBK() {
    static u16 plain[0x8000];
    for (u16 c = 0; c < 0x8000; c++) {
        plain[c] = AppSwap16(c | 0x8000);
    }
    return plain;
}
...
const u16* AppInitUTF16GBK() {
    static u16 utf162gbk[0x10000] = {0};
    u16 c = 0;
    u16 d = 0;
...
/**
* @param options = SNO_TO_LOWER | SNO_TO_HALF;
*/
void AppStrNormalizeUTF8(s8* text, u32 options) {
    const s8* pTransTable =
        (options & SNO_TO_LOWER) ? _pUpper2Lower : (
        (options & SNO_TO_UPPER) ? _pLower2Upper : GTB_PLAIN);
    const u16* pTransTable_utf16 =
        (options & SNO_TO_SIMPLIFIED) ? _pTrad2Simp_utf16 : GTB_PLAIN_UTF16;
    u32 i_from = 0, i_to = 0;

    while (text[i_from]) {
        if ((text[i_from] & 0x80) == 0) {
            text[i_to++] = pTransTable[(u32)text[i_from++]];
        } else if ((text[i_from] & 0xF0) == 0xE0
            && (text[i_from + 1] & 0xC0) == 0x80
            && (text[i_from + 2] & 0xC0) == 0x80) {
            u16 utf16 = (u16)(text[i_from] & 0x0F) << 12
                | (u16)(text[i_from + 1] & 0x3F) << 6
                | (u16)(text[i_from + 2] & 0x3F);
...
void AppStrNormalizeGBK(s8* text, u32 options) {
    const s8* pTransTable =
        (options & SNO_TO_LOWER) ? _pUpper2Lower :
        ((options & SNO_TO_UPPER) ? _pLower2Upper : GTB_PLAIN);
    const u16* pTransTable_gbk = (options & SNO_TO_SIMPLIFIED) ? _pTrad2Simp_gbk : GTB_PLAIN_GBK;
    u32 i_from = 0, i_to = 0, flag = 0;

    for (; text[i_from]; i_from++) {
        if (flag && (options & SNO_TO_HALF)) {
            if ((u8)text[i_from - 1] == 0xA1
                && (u8)text[i_from] == 0xA1) {
                flag = 0;
                text[i_from] = ' ';
            } else if ((u8)text[i_from - 1] == 0xA3 && (s8)text[i_from] < 0 && (u8)text[i_from] > 0xA0) {
                flag = 0;
                text[i_from] &= ~0x80;
            }
        }
        ...
        if (flag) {
            flag = 0;
            u16 tmp = pTransTable_gbk[AppCompU16(text[i_from - 1], text[i_from]) & ~0x8000];
            text[i_to++] = tmp & 0xFF;
            text[i_to++] = tmp >> 8;
...
usz AppGbkToUTF8(const s8* from, s8* out, usz osz) {
    if (from == nullptr || out == nullptr || osz < 2) {
        return 0;
    }
    osz -= 1;
    usz ret = 0;
    bool flag = false;
    for (usz slen = 0; 0 != from[slen] && ret < osz; ++slen) {
        if (flag) {
            flag = false;
            u16 tmp = GTB_GBK_UTF16[AppCompU16(from[slen - 1], from[slen]) & ~0x8000];
            if (tmp == 0) {
                continue;
            } else if (tmp >= 0x800) {
                if (ret + 3 > osz) {
                ...
                out[ret++] = 0xE0 | (tmp >> 12);
                out[ret++] = 0x80 | ((tmp >> 6) & 0x3F);
                out[ret++] = 0x80 | (tmp & 0x3F);
            } else if (tmp >= 0x80) {
                if (ret + 2 > osz) {
                    break;
                }
...
usz AppUTF8ToGBK(const s8* from, s8* out, usz osz) {
    if (from == nullptr || out == nullptr || osz < sizeof(s8)) {
        return 0;
    }
    osz -= sizeof(s16); //for tail \0
    usz slen = 0;
    usz ret = 0;
    for (slen = 0; 0 != from[slen] && ret < osz; ) {
        if ((u8)from[slen] < 0x80) {
            out[ret++] = from[slen++];
        } else if ((u8)from[slen] < 0xC2) {
            slen++;
        } else if ((u8)from[slen] < 0xE0) {
            if (0 == from[slen + 1]) {
                break;
            }
            u16 tmp = GTB_UTF16_GBK[((from[slen] & 0x1F) << 6) | (from[slen + 1] & 0x3F)];
            if (tmp) {
                out[ret++] = tmp >> 8;
                out[ret++] = tmp & 0xFF;
            }
            slen += 2;
        } else if ((u8)from[slen] < 0xF0) {
            if (0 == from[slen + 1] || 0 == from[slen + 2]) {
                break;
            }
            u16 tmp = GTB_UTF16_GBK[((from[slen] & 0x0F) << 12)
                | ((from[slen + 1] & 0x3F) << 6) | (from[slen + 2] & 0x3F)];
            ...
            if (0 == from[slen + 1] || 0 == from[slen + 2] || 0 == from[slen + 3]) {
                break;
            }
            slen += 4;
        }
